...
void toposort_recur(int node,vector<int>adj[],vector<int>&visited,stack<int>&st){
visited[node]=1;

             for(auto i:adj[node]){

     if(visited[i]==0){
          toposort_recur(i,adj,visited,st);
        }
...
vector<int> toposort_dfs(int n,vector<int>adj[]){
...
vector<int>ans=toposort_dfs(n,adj);
for(auto i:ans){
  cout<<i<<" ";
}
  return 0;
